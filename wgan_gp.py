...
def resnet_generator(inputs, noise=None, dim=DIM, nonlinearity=tf.nn.relu):
    ...
    output = ResidualBlock('Generator.Res3', 4*dim, 8*dim, 3, output, resample='down')
    output = ResidualBlock('Generator.Res4', 8*dim, 8*dim, 3, output, resample=None)
    output = ResidualBlock('Generator.Res5', 8*dim, 4*dim, 3, output, resample='up')
    output = ResidualBlock('Generator.Res6', 4*dim, 2*dim, 3, output, resample='up')
    output = ResidualBlock('Generator.Res7', 2*dim, 1*dim, 3, output, resample='up')
